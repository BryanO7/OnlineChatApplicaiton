...
class PrivateChatService(chat_unificado_pb2_grpc.ChatService):
    def __init__(self):
    ...
    def EstablishConnection(self, request, context):
        ...
        return chat_unificado_pb2.ConnectionResponse(message=f"Connection established with {target_username}")
    ...
    def SendMessage(self, request, context):
        ...
        if target_username in self.clients and username in self.clients[target_username]:
            ...
            return chat_unificado_pb2.MessageResponse(username=username, message=message)
        return chat_unificado_pb2.MessageResponse(username="Server", message="Failed to send message")
        ...
        if username in self.messages:
            while True:
                while self.messages[username]:
                    ...
                    yield chat_unificado_pb2.MessageResponse(username=sender, message=message)

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    chat_unificado_pb2_grpc.add_PrivateChatServiceServicer_to_server(PrivateChatService(), server)
...
class PrivateChatClient:
    def __init__(self, username, target_username, host='localhost', port=50052):
        ...
        self.stub = chat_unificado_pb2_grpc.PrivateChatServiceStub(self.channel)

    def establish_connection(self):
        request = chat_unificado_pb2.ConnectionRequest(username=self.username, target_username=self.target_username)
        response = self.stub.EstablishConnection(request)
        print(response.message)

    def send_message(self, message):
        request = chat_unificado_pb2.MessageRequest(username=self.username, target_username=self.target_username, message=message)
        response = self.stub.SendMessage(request)
        print(f"{response.username}: {response.message}")

    def receive_messages(self):
        request = chat_unificado_pb2.Empty()
