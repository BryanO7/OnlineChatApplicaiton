...
class ChatService(chat_pb2_grpc.ChatServiceServicer):
    ...
    def RegisterClient(self, request, context):
        ...
        return chat_pb2.ClientResponse(message=f"Cliente {username} registrado")

    def GetClientAddress(self, request, context):
        address = self.name_server.get_client_address(request.username)
        return chat_pb2.ClientResponse(message=f"Address: {address}")
    ...
    def SubscribeGroupChat(self, request, context):
        ...
        return chat_pb2.ChatResponse(message=f"Subscribed to chat {chat_id} with queue {queue_name}")

    def DiscoverChats(self, request, context):
        raw_chats = self.name_server.all_chats()
        chat_infos = []
        for chat_id in raw_chats:
            chat_info = chat_pb2.ChatInfo(
                chat_id=chat_id.decode()
            )
            chat_infos.append(chat_info)
        return chat_pb2.ChatListResponse(chats=chat_infos)
...
class NameServer:
    ...
    def all_chats(self):
        chats = self.redis_client.smembers("chats")
        return chats

    def chat_exists(self, chat_id):
        exists = self.redis_client.sismember("chats", chat_id)
        return exists
